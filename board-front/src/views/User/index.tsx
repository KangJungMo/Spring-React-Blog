...
export default function User() {
  ...
  const UserBottom = () => {
    ...
    const getUserBoardResponse = (responseBody: GetUserBoardListResponseDto | ResponseDto | null) => {
      if(!responseBody) return;
      const {code} = responseBody;
      if(code === 'NU') {
        alert('존재하지 않는 유저입니다.');
    ...
    //    effect: userEmail path variable 변경 될 때마다 실행할 함수    //
    useEffect(() => {
      if(!userEmail) return;
      getUserBoardListRequest(userEmail).then(getUserBoardResponse);
    }, [userEmail]);
            ...
            <div className='user-bottom-side-box'>
              <div className='user-bottom-side-card' onClick={onSideCardClickHandler}>
                <div className='user-bottom-side-container'>
                  ...
                  <div className='user-bottom-side-text'>{'글 쓰기'}</div>
                  </>:
                  <>
