import React, { ChangeEvent, EventHandler, useCallback, useEffect, useRef, useState } from 'react'
import './style.css';
...
export default function BoardDetail() {
  ...
  const getBoardRequestMemoized = useCallback(() => {
    if (!boardNumber) {
      navigator(MAIN_PATH());
      return;
    }
    getBoardRequest(boardNumber).then(getBoardResponse);
  }, [boardNumber, getBoardRequest, getBoardResponse, navigator]);

  //      effect: 게시물 번호 path variable이 바뀔때 마다 게시물 불러오기   //
  useEffect(() => {
    getBoardRequestMemoized();
  }, [getBoardRequestMemoized]);
  ...
  const BoardDetailBottom = () => {
    ...
    //      function: get favorite list response 처리함수   //
    ...
    const onCommentChangeHandeler = (event: ChangeEvent<HTMLTextAreaElement>) => {
        ...
        <div className='board-detail-bottom-comment-box'>
          ...
          <div className='board-detail-bottom-comment-input-box'>
            <div className='board-detail-bottom-comment-input-container'>
              <textarea ref={commentRef} className='board-detail-bottom-comment-textarea' placeholder='댓글을 작성해주세요.' value={comment} onChange={onCommentChangeHandeler} />
