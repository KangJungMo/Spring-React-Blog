...
import './style.css';
import { useBoardStroe, useLoginUserStore } from 'stores';
...
export default function BoardWrite() {
  ...
  //      state: 게시물 상태    //
  const { title, setTitle} = useBoardStroe();
  const { content, setContent} = useBoardStroe();
  const { boardImageFileList, setBoardImageFileList} = useBoardStroe();
  ...
  const getBoardReponse = (responseBody: GetBoardResponseDto | ResponseDto | null) => {
  ...
  useEffect(() => {
    ...
    getBoardRequest(boardNumber).then(getBoardReponse);
  }, [boardNumber]);
