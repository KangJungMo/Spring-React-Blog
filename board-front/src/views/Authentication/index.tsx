...
export default function Authentication() {
  ...
  const SignInCard = () => {
    ...
    //      state: 패스워드 타입 상태    //
    const[passwrdType, setPasswordType] = useState<'text' | 'password'>('password');
    ...
    //      event handler: 이메일 input 버튼 키다운 이벤트 처리 함수    //
    const onEmailKeyDownHandelr = (event: KeyboardEvent<HTMLInputElement>) => {
      if(event.key !== 'Enter') return; 
      if(!passwordRef.current) return;
      passwordRef.current.focus();
    }
       //      event handler: 이메일 input 버튼 키다운 이벤트 처리 함수    //
    const onPasswordDownHandelr = (event: KeyboardEvent<HTMLInputElement>) => {
      if(event.key !== 'Enter') return; 
      ...
      <div className='auth-card'>
        <div className='auth-card-box'>
          <div className='auth-card-top'>
            ...
            <InputBox ref={emailRef} label='이메일 주소' type='text' placeholder='이메일 주소를 입력해주세요.' error={error} value={email} onChange={onEmailChangeHandler} onKeyDown={onEmailKeyDownHandelr}/>
            <InputBox ref={passwordRef}label='패스워드' type={passwrdType} placeholder='비밀번호를 입력해 주세요.' error={error} value={password} onChange={onPasswordChangeHandler} icon={passwordButtonIcon} onButtonClick={onPasswordButtonClickHandler} onKeyDown={onPasswordDownHandelr}/>
          </div>
  ...
  //      state: telNumber 에러 상태    //
  const [isTelNumberError, setTelNumberError] = useState<boolean>(false);
  //      state: telNumber 에러 메세지 상태    //
  const [isTelNumberErrorMessage, setTelNumberErrorMessage] = useState<string>('');
  ...
  const onPasswordKeyDownHandelr = (event:KeyboardEvent<HTMLInputElement>) => {
    if(event.key !== 'Enter') return;
  ...
  const onPasswordCheckKeyDownHandelr = (event:KeyboardEvent<HTMLInputElement>) => {
    if(event.key !== 'Enter') return;
    ...
    <div className='auth-card'>
      <div className='auth-card-box'>
        <div className='auth-card-top'>
          ...
          {page === 1 && (
          <>
          <InputBox ref={emailRef} label='이메일 주소*' type='text' placeholder='이메일 주소를 입력해 주세요.' value={email} onChange={onEmailChangeHandler} error={isEmailError} message={emailErrorMessage} onKeyDown={onEmailKeyDownHandelr}/>
          <InputBox ref={passwordRef} label='비밀번호*' type={passwordType} placeholder='비밀번호를 입력해 주세요.' value={password} onChange={onPasswordChangeHandler} error={isPasswordError} message={passwordErrorMessage} icon={passwordButtonIcon} onButtonClick={onPasswordButtonClickHandler} onKeyDown={onPasswordKeyDownHandelr}/>
          <InputBox ref={passwordCheckRef} label='비밀번호 확인*' type={passwordCheckType} placeholder='비밀번호를 다시 입력해 주세요.' value={passwordCheck} onChange={onPasswordCheckChangeHandler} error={isPasswordCheckError} message={passwordCheckErrorMessage} icon={passwordCheckButtonIcon} onButtonClick={onPasswordCheckButtonClickHandler} onKeyDown={onPasswordCheckKeyDownHandelr}/>
          </>
          )}
          {page === 2 && (
          <>
          ...
          <InputBox ref={telNumberRef} label='휴대 폰번호*' type='text' placeholder='핸드폰 번호를 입력해주세요' value={telNumber} onChange={onTelNumberChangeHandler} error={isTelNumberError} message={isTelNumberErrorMessage} onKeyDown={onTelNumberKeyDownHandler}/>
      ...
      <div className='auth-container'>
        <div className='auth-jumbotron-box'>
          <div className='auth-jumbotron-content'>
            ...
            <div className='auth-jumbotron-text'>{'KANGS BOARD 입니다.'}</div>
            </div>
          </div>
        </div>
